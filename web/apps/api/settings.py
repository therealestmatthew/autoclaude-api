...
`uv run ft-autoclaude-api` story without a config file.
"""
...
def _repo_root_default() -> Path:
    ...
    the app from anywhere as long as `FT_AUTOCLAUDE_REPO_ROOT` is set.
    """
...
class Settings:
    ...
    def from_env(cls) -> Settings:
        repo_root = Path(
            os.environ.get("FT_AUTOCLAUDE_REPO_ROOT", str(_repo_root_default()))
        ).resolve()
        host = os.environ.get("FT_AUTOCLAUDE_API_HOST", "127.0.0.1")
        port = int(os.environ.get("FT_AUTOCLAUDE_API_PORT", "8080"))
        origins_env = os.environ.get(
            "FT_AUTOCLAUDE_API_CORS_ORIGINS",
            "http://localhost:3000,http://127.0.0.1:3000,"
            "http://localhost:3001,http://127.0.0.1:3001",
        )
        cors_origins = tuple(o.strip() for o in origins_env.split(",") if o.strip())
        log_level = os.environ.get("FT_AUTOCLAUDE_API_LOG_LEVEL", "info")

        dsn = os.environ.get("FT_AUTOCLAUDE_INDEX_DSN", "").strip() or None
        reconcile_interval = float(
            os.environ.get("FT_AUTOCLAUDE_INDEX_RECONCILE_INTERVAL", "60")
        )
        # Default ON: dev story is "uv run ft-autoclaude-api just works on a
        # fresh checkout." Production / CI can set it to 0 to keep schema
        # migrations as an explicit deploy step.
        auto_migrate = os.environ.get("FT_AUTOCLAUDE_INDEX_AUTO_MIGRATE", "1").strip() in {
            "1",
            "true",
            "yes",
            "on",
