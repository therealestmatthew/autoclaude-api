"""Brand catalog endpoints (read-only).

Brands are catalog assets with kind=brand, indexed from /brands/<slug>/brand.md
into the 'brand' bucket. This router mirrors the engagements pattern.
"""
...
def list_brands(
    ...
    snapshot = index.get()
    records = [r for r in snapshot.records if r.bucket == "brand"]
...
def get_brand(slug: str, index: CachedIndex = Depends(get_index)) -> AssetDetail:
    snapshot = index.get()
    for r in snapshot.records:
        if r.bucket == "brand" and r.slug == slug:
            return to_detail(r)
